...
class SiteAddressTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.address)
}
...
class SiteAddressLabelTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = {
        ...
        val site = request.context.siteDao.get
...
class SiteNameTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.name)
}

@Tag("comp_country")
class SiteCountryTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.country)
}

@Tag("comp_email")
class SiteEmailTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) =
        Option(request.context.installationDao.get.email).map(email => "<a href='mailto:" + email + "'>" + email + "</a>")
}

@Tag("comp_telephone")
class SitePhoneTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.telephone)
}

@Tag("comp_postcode")
class SitePostcodeTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.postcode)
}

@Tag("comp_vat")
class SiteVatTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.vatNumber)
}

@Tag("comp_number")
class SiteCompanyNumberTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = Option(request.context.installationDao.get.companyNumber)
}

@Tag("comp_postcode_gmap")
class SiteGoogleMapTag extends ScalapressTag with TagBuilder {
    def render(request: ScalapressRequest, params: Map[String, String]) = {
        val site = request.context.installationDao.get
