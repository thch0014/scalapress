...
import com.cloudray.scalapress.folder.{FolderOrdering, Folder}
...
class FoldersWidgetTest extends FunSuite with MockitoSugar with OneInstancePerTest {
  ...
  test("widget renders using folder positions when manual ordering set") {

    child1.position = 3
    child2.position = 1
    child4.position = 2
    parent.folderOrdering = FolderOrdering.Manual
    val children = widget._children(parent)
    assert(child2 === children(0))
    assert(child4 === children(1))
    assert(child1 === children(2))
  }

  test("widget renders folders alphabetically when alphabetical ordering set") {

    child1.position = 1
    child2.position = 2
    child4.position = 4
    parent.folderOrdering = FolderOrdering.Alphabetical
    val children = widget._children(parent)
    assert(child1 === children(0))
    assert(child2 === children(1))
    assert(child4 === children(2))
  }
