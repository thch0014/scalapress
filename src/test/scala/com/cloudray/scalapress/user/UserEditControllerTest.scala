...
class UserEditControllerTest extends FunSuite with OneInstancePerTest with MockitoSugar {
    ...
    val controller = new UserEditController
    controller.userDao = mock[UserDao]
    controller.passwordEncoder = mock[PasswordEncoder]
    Mockito.when(controller.passwordEncoder.encodePassword(Matchers.anyString, Matchers.anyString)).thenReturn("encrypted")
