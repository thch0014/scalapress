...
class PaymentFormRendererTest extends FunSuite with MockitoSugar with OneInstancePerTest {
  ...
  context.paymentPluginDao = mock[PaymentPluginDao]
  context.installationDao = mock[InstallationDao]
  val renderer = new PaymentFormRenderer(context)
  ...
  test("renderer renders all enabled payments") {
    ...
    Mockito.when(context.paymentPluginDao.enabled).thenReturn(Seq(plugin1, plugin2))
  ...
  test("renderer renders params for each plugin") {
    ...
    Mockito.when(context.paymentPluginDao.enabled).thenReturn(Seq(plugin1))
    val form = renderer.renderPaymentForm(purchase).toString()
