...
class ObjectSorterTest extends FunSuite with MockitoSugar with OneInstancePerTest {
  ...
  test("Newest sort sorts by id asc") {
    val sorted = ObjectSorter.sort(objs, Sort.Newest, None)
    assert(Seq(obj3, obj1, obj5, obj2, obj4) === sorted)
  }

  test("Oldest sort sorts by id asc") {
    val sorted = ObjectSorter.sort(objs, Sort.Oldest, None)
    assert(Seq(obj4, obj2, obj5, obj1, obj3) === sorted)
  }

  test("Attribute sort uses values from attributes") {
    val sorted = ObjectSorter.sort(objs, Sort.Attribute, Some(av1.attribute))
    assert(Seq(obj1, obj5, obj4, obj3, obj2) === sorted)
  }

  test("Attribute sort without specifying attribute defaults to name sort") {
    val sorted = ObjectSorter.sort(objs, Sort.Attribute, None)
    assert(Seq(obj1, obj3, obj5, obj4, obj2) === sorted)
  }

  test("random maintains order for the same hash code") {
    val initial = ObjectSorter.sort(objs, Sort.Random, None, "someseed".hashCode)
    for ( i <- 1 to 5 )
      assert(ObjectSorter.sort(objs, Sort.Random, None, "someseed".hashCode) === initial)
    for ( i <- 1 to 5 )
      assert(ObjectSorter.sort(objs, Sort.Random, None, "otherseed".hashCode) != initial)
  }
}
