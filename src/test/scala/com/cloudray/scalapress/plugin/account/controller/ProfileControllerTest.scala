...
class ProfileControllerTest extends FlatSpec with MockitoSugar with OneInstancePerTest {
  ...
  "a account controller" should "persist user when required fields are set" in {
    controller.update(req, profile, null)
    Mockito.verify(controller.accountDao).save(acc)
  }

  "a account controller" should "not persist user when required fields are not set" in {
    profile.name = null
